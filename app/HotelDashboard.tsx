...
type Data = { user: { name: string; email: string; role: string }; floors: Floor[]; rooms: Room[]; events: HotelEvent[] };
...
export default function HotelDashboard() {
  ...
  const [modal, setModal] = useState<"room" | "checkin" | "event" | "transfer" | "edit" | "print" | null>(null);
  ...
  return (
    <main className="app-shell">
      ...
      <section className="content">
        ...
        {view === "configuracion" && <section className="settings-grid"><article className="panel settings-card"><span>🏨</span><h2>Estructura del hotel</h2><p>Actualmente hay {data.floors.length} pisos y {data.rooms.length} habitaciones. La numeración y características son editables desde cada habitación.</p><div className="floor-list">{data.floors.map((floor) => <div key={floor.id}><b>{floor.name}</b><span>{data.rooms.filter((room) => room.floor_id === floor.id).length} habitaciones</span></div>)}</div></article><article className="panel settings-card"><span>👥</span><h2>Roles del personal</h2><p>Los accesos previstos distinguen al propietario, administración y recepción.</p><div className="role-list"><b>Propietario</b><small>Control total y configuración</small><b>Administrador</b><small>Gestión operativa completa</small><b>Recepción</b><small>Ingresos, salidas, actas y eventos</small></div></article></section>}
      </section>

      {modal && selected && <Modal onClose={() => setModal(null)} wide={modal === "room" || modal === "print"}>
        {modal === "room" && <RoomDetail room={selected} data={data} onAction={(next) => setModal(next)} onCheckout={() => action({ action: "checkout", roomId: selected.id })} busy={busy} />}
        ...
        {modal === "print" && <DeliveryAct room={selected} onPrint={() => window.print()} />}
      </Modal>}
    </main>
  );
}
...
function RoomDetail({ room, data, onAction, onCheckout, busy }: { room: Room; data: Data; onAction: (modal: "checkin" | "event" | "transfer" | "edit" | "print") => void; onCheckout: () => void; busy: boolean }) {
  ...
  return <><div className="detail-title"><div><p className="eyebrow">{data.floors.find((floor) => floor.id === room.floor_id)?.name}</p><h2>Habitación {room.number}</h2></div><span className={`big-status status-${room.status.toLowerCase()}`}>{labels[room.status]}</span></div>
    <div className="detail-grid"><div className="detail-main"><section className="info-box"><h3>{room.guest_name ? "Estadía activa" : "Información de habitación"}</h3>{room.guest_name ? <div className="guest-profile"><span>{room.guest_name.slice(0, 1)}</span><div><b>{room.guest_name}</b><p>CI {room.guest_ci || "no registrado"} · {room.guest_count || 1} personas</p><small>Ingreso: {new Intl.DateTimeFormat("es-BO", { dateStyle: "medium", timeStyle: "short" }).format(new Date(room.check_in!))}</small></div></div> : <div className="room-facts"><div><small>Tipo</small><b>{room.type}</b></div><div><small>Capacidad</small><b>{room.capacity} personas</b></div><div><small>Estado</small><b>{labels[room.status]}</b></div></div>}</section>
      ...
      <aside className="action-stack"><h3>Acciones</h3>{room.status === "DISPONIBLE" && <button className="primary" onClick={() => onAction("checkin")}>Registrar ingreso</button>}{room.status === "OCUPADA" && <><button onClick={() => onAction("print")}>Imprimir acta de entrega</button><button onClick={() => onAction("transfer")}>Cambiar habitación</button><button className="danger-light" disabled={busy} onClick={onCheckout}>Registrar salida</button></>}<button onClick={() => onAction("event")}>Añadir evento</button><button onClick={() => onAction("edit")}>Editar habitación</button></aside></div></>;
}
...
function DeliveryAct({ room, onPrint }: { room: Room; onPrint: () => void }) { const items = ["Puerta, cerradura y llaves", "Paredes, techo y piso", "Ventanas y cortinas", "Cama, colchón y ropa de cama", "Mesas, sillas y ropero", "Televisor y control remoto", "Baño e instalaciones sanitarias", "Iluminación y tomacorrientes"]; return <div className="print-wrapper"><div className="print-toolbar"><div><p className="eyebrow">Documento listo</p><h2>Acta de entrega</h2></div><button className="primary" onClick={onPrint}>Imprimir acta</button></div><article className="acta"><header><span className="brand-mark">A</span><div><h2>HOTEL ASAEL</h2><p>ACTA DE ENTREGA DE HABITACIÓN</p></div><b>Habitación {room.number}</b></header><section className="acta-meta"><div><small>Huésped titular</small><strong>{room.guest_name}</strong></div><div><small>CI</small><strong>{room.guest_ci || "________________"}</strong></div><div><small>Fecha de ingreso</small><strong>{room.check_in ? new Intl.DateTimeFormat("es-BO", { dateStyle: "medium" }).format(new Date(room.check_in)) : ""}</strong></div><div><small>Tipo de estadía</small><strong>{labels[room.stay_type || ""]}</strong></div></section><p>Se deja constancia de la entrega de la habitación y de los siguientes elementos, cuyo estado ha sido revisado por ambas partes:</p><table><thead><tr><th>Elemento verificado</th><th>Bueno</th><th>Observado</th><th>Observaciones</th></tr></thead><tbody>{items.map((item) => <tr key={item}><td>{item}</td><td>□</td><td>□</td><td /></tr>)}</tbody></table><div className="acta-notes"><b>Observaciones generales:</b></div><footer><div><span /><b>Firma del huésped</b><small>Nombre y CI</small></div><div><span /><b>Firma del encargado</b><small>Hotel ASAEL</small></div></footer></article></div>; }
