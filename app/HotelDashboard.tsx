...
export default function HotelDashboard() {
  ...
  const [modal, setModal] = useState<"room" | "checkin" | "event" | "transfer" | "edit" | "inventory" | "inspection" | "print" | "correction" | "occupants" | "guest" | "primary" | null>(null);
  ...
  const pendingApprovals = data.changeRequests.filter((request) => request.status === "PENDIENTE").length + data.primaryTransfers.filter((request) => request.status === "PENDIENTE").length;
...
function ApprovalsView({ data, busy, onAction }: { data: Data; busy: boolean; onAction: (payload: Record<string, unknown>) => Promise<unknown> }) {
  ...
  const allRequests = [...data.changeRequests, ...data.primaryTransfers];
