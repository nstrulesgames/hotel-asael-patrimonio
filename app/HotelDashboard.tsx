...
export default function HotelDashboard() {
  ...
  const [modal, setModal] = useState<"room" | "checkin" | "event" | "transfer" | "edit" | "inventory" | "inspection" | "print" | "correction" | "occupants" | null>(null);
  const [inspectionKind, setInspectionKind] = useState<InspectionKind>("ENTREGA");
  const [busy, setBusy] = useState(false);
  const [notice, setNotice] = useState("");
  const [companions, setCompanions] = useState<Companion[]>([]);
  const [view, setView] = useState<"habitaciones" | "tareas" | "aprobaciones" | "auditoria" | "actividad" | "configuracion">("habitaciones");
    ...
    <main className="app-shell">
      ...
      <section className="content">
        <header className="topbar">
          <div><p className="eyebrow">Hotel ASAEL · {new Intl.DateTimeFormat("es-BO", { weekday: "long", day: "numeric", month: "long" }).format(new Date())}</p><h1>{view === "habitaciones" ? "Estado del hotel" : view === "tareas" ? "Tareas operativas" : view === "aprobaciones" ? "Correcciones y aprobaciones" : view === "auditoria" ? "Auditoría administrativa" : view === "actividad" ? "Bitácora operativa" : "Configuración del hotel"}</h1></div>
...
function RoomDetail({ room, data, onReload, onAction, onOpenTasks, onInspection, onPrint, onCheckout, onWorkflow, busy }: { room: Room; data: Data; onReload: () => Promise<void>; onAction: (modal: "checkin" | "event" | "transfer" | "edit" | "inventory" | "correction" | "occupants") => void; onOpenTasks: () => void; onInspection: (kind: InspectionKind) => void; onPrint: (kind: "ENTREGA" | "DEVOLUCION") => void; onCheckout: () => void; onWorkflow: (action: "cleaning_start" | "cleaning_complete" | "cleaning_reopen") => void; busy: boolean }) {
  const [referenceTime] = useState(() => Date.now());
