...
type StoreProduct = {
  id: number; sku: string; name: string; category: string; item_type: "PRODUCTO" | "SERVICIO"; purchase_unit: string; sale_unit: string;
...
type StoreMovement = { id: number; product_name: string; sale_unit: string; movement_type: string; quantity: number; total_cost_cents: number | null; from_location_name?: string; to_location_name?: string; reason: string; responsible: string; supplier?: string; receipt_number?: string; receipt_filename?: string; created_by: string; created_at: string };
...
type StoreData = { user: { id: number; name: string; role: string }; products: StoreProduct[]; movements: StoreMovement[]; expiring: ExpiringBatch[]; replenishments: Replenishment[]; pendingLimitCents: number; servicesEnabled: boolean };
...
export default function StoreView() {
  ...
  const physicalProducts = useMemo(() => activeProducts.filter((product) => product.item_type !== "SERVICIO"), [activeProducts]);
  const lowStock = useMemo(() => physicalProducts.filter((product) => product.reception_stock <= product.minimum_stock), [physicalProducts]);
  const receptionUnits = physicalProducts.reduce((sum, product) => sum + product.reception_stock, 0);
  ...
  return <section className="store-page">
    ...
    <div className="store-metrics">
      <article><span>Catálogo activo</span><strong>{activeProducts.length}</strong><small>{physicalProducts.length} productos · {activeProducts.length - physicalProducts.length} servicios</small></article>
    ...
    {admin && <div className="store-forms-grid">
      <ProductForm editing={editing} servicesEnabled={data.servicesEnabled} busy={busy} onCancel={() => setEditing(null)} onSave={async (payload) => { const ok = await action(payload); if (ok) setEditing(null); return ok; }} />
      <StockEntryForm products={physicalProducts} busy={busy} onSave={action} onReload={load} onNotice={setNotice} />
      <TransferForm products={physicalProducts} busy={busy} onSave={action} />
      <StockAdjustmentForm products={physicalProducts} busy={busy} onSave={action} />
    </div>}

    <div className="store-request-grid"><ReplenishmentForm products={physicalProducts} busy={busy} onSave={action} />{admin && <CommercialSettingsForm pendingLimitCents={data.pendingLimitCents} servicesEnabled={data.servicesEnabled} owner={data.user.role === "PROPIETARIO"} busy={busy} onSave={action} />}</div>
    ...
    <div className="store-content-grid">
      <div className="panel store-catalog">
        ...
        <div className="store-table-wrap"><table className="store-table"><thead><tr><th>Producto</th><th>Precio</th>{admin && <th>Almacén</th>}<th>Recepción</th>{admin && <th>Costo prom.</th>}<th>Próximo venc.</th>{admin && <th></th>}</tr></thead><tbody>
          ...
          {data.products.map((product) => <tr key={product.id} className={!product.active ? "inactive" : product.reception_stock <= product.minimum_stock ? "low" : ""}>
            <td><div className="product-name"><b>{product.name}</b><small>{product.sku} · {product.item_type === "SERVICIO" ? "Servicio sin stock" : `${product.category} · ${product.units_per_purchase} ${product.sale_unit} por ${product.purchase_unit}`}</small>{!product.active && <em>Inactivo</em>}</div></td>
            <td>{money(product.sale_price_cents)}</td>
            {admin && <td>{product.item_type === "SERVICIO" ? "—" : <><strong>{product.main_stock}</strong> <small>{product.sale_unit}</small></>}</td>}
            <td>{product.item_type === "SERVICIO" ? "No aplica" : <><strong>{product.reception_stock}</strong> <small>{product.sale_unit}</small>{product.reception_stock <= product.minimum_stock && <span className="stock-low-label">Mín. {product.minimum_stock}</span>}</>}</td>
      ...
      <div className="panel store-expiry">
        ...
        {data.expiring.length === 0 ? <p className="empty-copy">No existen lotes próximos a vencer.</p> : <div className="expiry-list">{data.expiring.map((batch) => { const days = daysUntil(batch.expires_on); const tier = days <= 7 ? "urgent" : days <= 15 ? "warning" : "notice"; return <article key={batch.id} className={tier}><div><b>{batch.product_name}</b><small>{batch.location_name} · {batch.quantity} unidades · {days < 0 ? "vencido" : days === 0 ? "vence hoy" : `${days} días`}</small></div><time>{date(batch.expires_on)}</time></article>; })}</div>}
      </div>
    </div>
    ...
    <div className="panel store-movements">
      ...
      <div className="movement-list">{data.movements.length === 0 ? <p className="empty-copy">Los ingresos y transferencias aparecerán aquí.</p> : data.movements.map((movement) => <article key={movement.id}>
        ...
        <div><b>{movementLabels[movement.movement_type] || movement.movement_type} · {movement.product_name}</b><small>{movement.from_location_name ? `${movement.from_location_name} → ` : ""}{movement.to_location_name || "Sin destino"} · {movement.reason}</small><small>{movement.responsible} · registrado por {movement.created_by}{movement.supplier ? ` · Proveedor: ${movement.supplier}` : ""}{movement.receipt_number ? ` · Comp.: ${movement.receipt_number}` : ""}</small>{admin && movement.receipt_filename && <a href={`/api/stock-entry-evidence?movementId=${movement.id}`} target="_blank" rel="noreferrer">↗ Ver comprobante de compra</a>}</div>
...
function ProductForm({ editing, servicesEnabled, busy, onCancel, onSave }: { editing: StoreProduct | null; servicesEnabled: boolean; busy: boolean; onCancel: () => void; onSave: (payload: Record<string, unknown>) => Promise<boolean> }) {
  const submit = async (event: FormEvent<HTMLFormElement>) => {
    event.preventDefault(); const form = event.currentTarget; const values = new FormData(form);
    const ok = await onSave({ action: "product_save", productId: editing?.id, sku: values.get("sku"), name: values.get("name"), category: values.get("category"), itemType: values.get("itemType"), purchaseUnit: values.get("purchaseUnit"), saleUnit: values.get("saleUnit"), unitsPerPurchase: Number(values.get("unitsPerPurchase")), salePriceCents: Math.round(Number(values.get("salePrice")) * 100), minimumStock: Number(values.get("minimumStock")), tracksExpiry: values.get("tracksExpiry") === "on" });
    if (ok && !editing) form.reset();
  };
  return <form className="panel store-form" onSubmit={submit} key={editing?.id || "new"}><div className="store-form-title"><div><span>01</span><div><h3>{editing ? "Editar artículo" : "Nuevo artículo"}</h3><p>Productos físicos o servicios bajo llave.</p></div></div>{editing && <button type="button" className="text-button" onClick={onCancel}>Cancelar</button>}</div>
    <label>Tipo<select name="itemType" defaultValue={editing?.item_type || "PRODUCTO"}><option value="PRODUCTO">Producto con existencias</option>{servicesEnabled && <option value="SERVICIO">Servicio sin existencias</option>}</select></label>
...
function StockEntryForm({ products, busy, onSave, onReload, onNotice }: { products: StoreProduct[]; busy: boolean; onSave: (payload: Record<string, unknown>) => Promise<unknown>; onReload: () => Promise<void>; onNotice: (message: string) => void }) {
  const [productId, setProductId] = useState(""); const [receiptFile, setReceiptFile] = useState<File | null>(null); const selected = products.find((product) => String(product.id) === productId);
  const submit = async (event: FormEvent<HTMLFormElement>) => { event.preventDefault(); const form = event.currentTarget; const values = new FormData(form); const result = await onSave({ action: "stock_entry", productId: Number(values.get("productId")), purchaseQuantity: Number(values.get("purchaseQuantity")), purchaseCostCents: Math.round(Number(values.get("purchaseCost")) * 100), expiresOn: values.get("expiresOn"), reason: values.get("reason"), responsible: values.get("responsible"), supplier: values.get("supplier"), receiptNumber: values.get("receiptNumber") }) as { movementId?: number } | false; if (!result) return; if (receiptFile && result.movementId) { const upload = new FormData(); upload.set("movementId", String(result.movementId)); upload.set("file", receiptFile); const response = await fetch("/api/stock-entry-evidence", { method: "POST", body: upload }); const uploaded = await jsonResponse(response); if (!response.ok) { onNotice(`El ingreso quedó registrado, pero el comprobante no se cargó: ${uploaded.error || "intenta nuevamente"}`); return; } await onReload(); } form.reset(); setProductId(""); setReceiptFile(null); };
  return <form className="panel store-form" onSubmit={submit}><div className="store-form-title"><div><span>02</span><div><h3>Ingreso al almacén</h3><p>Registra compras y su costo real.</p></div></div></div>
    ...
    <div className="form-two"><label>Proveedor (opcional)<input name="supplier" placeholder="Nombre o empresa" /></label><label>N.º comprobante (opcional)<input name="receiptNumber" placeholder="Factura, recibo o nota" /></label></div>
    <label>Fotografía o PDF del comprobante<input type="file" accept="image/*,application/pdf" onChange={(event) => setReceiptFile(event.target.files?.[0] || null)} /><small>Opcional · máximo 15 MB</small></label><label>Motivo<input name="reason" defaultValue="Compra de mercadería" required /></label><label>Responsable físico<input name="responsible" placeholder="Persona que recibió" required /></label>
...
function CommercialSettingsForm({ pendingLimitCents, servicesEnabled, owner, busy, onSave }: { pendingLimitCents: number; servicesEnabled: boolean; owner: boolean; busy: boolean; onSave: (payload: Record<string, unknown>) => Promise<boolean> }) {
  ...
  return <form className="panel store-form commercial-settings-form" onSubmit={submit}><div className="store-form-title"><div><span>⚙</span><div><h3>Control comercial</h3><p>Límite de consumos y módulos protegidos.</p></div></div></div><label>Límite pendiente por estadía (Bs)<input name="pendingLimit" type="number" min="0" max="10000" step="0.01" defaultValue={(pendingLimitCents / 100).toFixed(2)} required /></label><small>Valor vigente: {money(pendingLimitCents)}. Los saldos ya existentes no se modifican.</small><button className="primary" disabled={busy}>{busy ? "Guardando…" : "Actualizar límite"}</button>{owner ? <button type="button" className={servicesEnabled ? "danger-light" : "service-toggle"} disabled={busy} onClick={() => onSave({ action: "services_toggle", enabled: !servicesEnabled })}>{servicesEnabled ? "Desactivar servicios" : "Activar módulo de servicios"}</button> : <small>El módulo de servicios está {servicesEnabled ? "activo" : "inactivo"}; solo el Propietario puede cambiarlo.</small>}</form>;
}
