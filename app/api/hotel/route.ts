...
export async function POST(request: Request) {
  ...
  if (body.action === "checkout") {
    ...
    const pendingSales = await env.DB.prepare("SELECT COUNT(*) AS sale_count, COALESCE(SUM(total_cents), 0) AS pending_cents FROM sales WHERE stay_id = ? AND status = 'PENDIENTE'").bind(stay.id).first<{ sale_count: number; pending_cents: number }>();
