...
type Product = { id: number; sku: string; name: string; category: string; item_type: "PRODUCTO" | "SERVICIO"; sale_unit: string; sale_price_cents: number; reception_stock: number; active: number };
...
type Sale = { id: number; sale_number: string; sale_type: "HUESPED" | "DIRECTA"; stay_id?: number; room_number?: string; customer_name?: string; customer_ci?: string; customer_phone?: string; status: "PAGADA" | "PENDIENTE" | "ANULADA" | "DEVUELTA" | "CORTESIA_PENDIENTE" | "CORTESIA_RECHAZADA"; payment_method: string; cash_session_id?: number; total_cents: number; paid_cents: number; returned_cents: number; balance_cents: number; print_count: number; created_by_name: string; created_at: string; cancellation_reason?: string; courtesy_reviewed_by_name?: string; courtesy_review_note?: string };
...
type Payment = { id: number; sale_id: number; payment_method: string; amount_cents: number; reference: string; received_by_name: string; received_at: string };
type PaymentEvidence = { id: number; sale_payment_id: number; filename: string; content_type: string; uploaded_by_name: string; created_at: string };
type ReportData = { periods: Array<{ period: string; net_sales_cents: number; sale_count: number }>; products: Array<{ product_id: number; product_name: string; units: number; revenue_cents: number; cost_cents: number }>; workers: Array<{ created_by_name: string; sale_count: number; sales_cents: number }>; paymentMethods: Array<{ payment_method: string; payment_count: number; amount_cents: number }>; statuses: Array<{ status: string; sale_count: number; total_cents: number }>; guestsRooms: Array<{ consumer_name: string; room_number: string; sale_count: number; total_cents: number }> };
type PosData = { user: { id: number; name: string; role: string }; products: Product[]; activeStays: ActiveStay[]; occupants: Occupant[]; recentSales: Sale[]; pendingByStay: Array<{ stay_id: number; sale_count: number; pending_cents: number }>; pendingLimitCents: number; servicesEnabled: boolean; currentCashSession?: CashSession; cashSessions: CashSession[]; saleItems: SaleItem[]; payments: Payment[]; paymentEvidences: PaymentEvidence[]; returns: Array<Record<string, unknown>>; reports: ReportData };
...
const saleStatusLabels: Record<string, string> = { PAGADA: "Pagada", PENDIENTE: "Pendiente", ANULADA: "Anulada", DEVUELTA: "Devuelta", CORTESIA_PENDIENTE: "Cortesía por aprobar", CORTESIA_RECHAZADA: "Cortesía rechazada" };
...
async function uploadPaymentEvidences(paymentId: number, files: File[]) {
  if (!paymentId || !files.length) return [];
  const form = new FormData(); form.set("salePaymentId", String(paymentId)); files.forEach((file) => form.append("files", file));
  const response = await fetch("/api/payment-evidence", { method: "POST", body: form });
  const result = await readJson(response);
  if (!response.ok) throw new Error(result.error || "No se pudo cargar el respaldo digital.");
  return result.evidenceIds as number[];
}
...
export default function PosView() {
  ...
  const availableProducts = useMemo(() => (data?.products || []).filter((product) => Boolean(product.active) && (product.item_type === "SERVICIO" ? Boolean(data?.servicesEnabled) : Number(product.reception_stock) > 0) && `${product.name} ${product.sku} ${product.category}`.toLowerCase().includes(search.toLowerCase())), [data, search]);
  ...
  const changeQuantity = (product: Product, next: number) => setCart((current) => {
    const quantity = Math.max(0, Math.min(Math.floor(next), product.item_type === "SERVICIO" ? 1000 : Number(product.reception_stock)));
  ...
  const submit = async (event: FormEvent) => {
    ...
    try {
      const response = await fetch("/api/store", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ action: "sale_create", saleType, stayId: Number(stayId), consumerGuestId: Number(consumerGuestId) || undefined, customerName, customerCi, customerPhone, paymentMethod, paymentReference, pendingOverride, notes, items: cartItems.map((item) => ({ productId: item.product.id, quantity: item.quantity })) }) });
      const result = await readJson(response);
      if (!response.ok) throw new Error(result.error || "No se pudo registrar la venta.");
      let evidenceWarning = "";
      if (result.paymentId && paymentEvidenceFiles.length) {
        try { await uploadPaymentEvidences(result.paymentId, paymentEvidenceFiles); }
        catch { evidenceWarning = " El pago quedó guardado, pero el respaldo digital no pudo cargarse."; }
      }
      setCart({}); setNotes(""); setCustomerName(""); setCustomerCi(""); setCustomerPhone(""); setPaymentReference(""); setPaymentEvidenceFiles([]); setPendingOverride(false);
      if (result.receiptUrl) setLastSale({ saleNumber: result.saleNumber, receiptUrl: result.receiptUrl });
      setNotice(result.status === "CORTESIA_PENDIENTE" ? `${result.saleNumber}: solicitud de cortesía enviada a Administración. El stock se descontará al aprobarla.` : `${result.saleNumber} registrada por ${money(result.totalCents)}.${evidenceWarning}`);
      await load();
  ...
  const reviewCourtesy = async (sale: Sale, decision: "APROBADA" | "RECHAZADA") => {
    const reviewNote = window.prompt(decision === "APROBADA" ? "Nota obligatoria para aprobar la cortesía:" : "Motivo obligatorio para rechazar la cortesía:");
    if (!reviewNote?.trim()) return;
    setBusy(true); setNotice("");
    try {
      const response = await fetch("/api/store", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ action: "courtesy_review", saleId: sale.id, decision, reviewNote }) });
      const result = await readJson(response);
      if (!response.ok) throw new Error(result.error || "No se pudo revisar la cortesía.");
      setNotice(decision === "APROBADA" ? "Cortesía aprobada; se descontó el stock y ya puede imprimirse." : "Cortesía rechazada sin afectar existencias.");
      await load();
    } catch (error) { setNotice(error instanceof Error ? error.message : "No se pudo revisar la cortesía."); }
    finally { setBusy(false); }
  };
  ...
  return <section className="pos-page">
    ...
    <div className="pos-workspace">
      <section className="panel pos-products"><div className="panel-head"><div><h2>Productos y servicios</h2><p>Precio único, sin modificación desde recepción.</p></div><label className="pos-search">⌕<input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Buscar producto o servicio…" /></label></div><div className="pos-product-grid">{availableProducts.map((product) => <article key={product.id} className={product.item_type === "SERVICIO" ? "service-card" : ""}><div><span>{product.item_type === "SERVICIO" ? "SERVICIO" : product.category}</span><h3>{product.name}</h3><small>{product.sku} · {product.item_type === "SERVICIO" ? "sin existencias" : `${product.reception_stock} ${product.sale_unit}`}</small></div><strong>{money(product.sale_price_cents)}</strong><button onClick={() => changeQuantity(product, (cart[product.id] || 0) + 1)}>＋ Agregar</button></article>)}{!availableProducts.length && <p className="pos-empty">No hay productos con stock ni servicios activos para esta búsqueda.</p>}</div></section>
      ...
      <form className="panel pos-cart" onSubmit={submit}><div className="pos-cart-title"><div><span>Venta actual</span><h2>{cartItems.length} producto(s)</h2></div><strong>{money(totalCents)}</strong></div><div className="pos-cart-lines">{cartItems.map(({ product, quantity }) => <div key={product.id}><div><b>{product.name}</b><small>{money(product.sale_price_cents)} c/u</small></div><div className="quantity-control"><button type="button" onClick={() => changeQuantity(product, quantity - 1)}>−</button><span>{quantity}</span><button type="button" onClick={() => changeQuantity(product, quantity + 1)}>＋</button></div><strong>{money(quantity * product.sale_price_cents)}</strong></div>)}{!cartItems.length && <p className="pos-empty">Agrega productos para iniciar la venta.</p>}</div>
        ...
        {saleType === "HUESPED" ? <div className="pos-fields"><label>Estadía activa<select required value={stayId} onChange={(event) => { setStayId(event.target.value); setConsumerGuestId(""); }}><option value="">Seleccionar…</option>{data.activeStays.map((stay) => <option value={stay.id} key={stay.id}>Hab. {stay.room_number} · {stay.primary_guest_name}</option>)}</select></label><label>Consumidor dentro de la estadía<select value={consumerGuestId} onChange={(event) => setConsumerGuestId(event.target.value)}><option value="">Titular responsable</option>{stayOccupants.map((occupant) => <option value={occupant.id} key={occupant.id}>{occupant.full_name}{occupant.is_primary ? " · Titular" : ""}</option>)}</select></label>{selectedStay && <p className="pos-stay-note"><b>Responsable:</b> {selectedStay.primary_guest_name} · Habitación {selectedStay.room_number}<br /><b>Pendiente actual:</b> {money(currentPending)}</p>}</div> : <div className="pos-fields"><label>Nombre del cliente (opcional)<input value={customerName} onChange={(event) => setCustomerName(event.target.value)} placeholder="Consumidor final" /></label><div className="form-two"><label>CI (opcional)<input value={customerCi} onChange={(event) => setCustomerCi(event.target.value)} /></label><label>Celular (opcional)<input value={customerPhone} onChange={(event) => setCustomerPhone(event.target.value)} /></label></div></div>}
        <div className="pos-fields"><label>Forma de pago<select value={paymentMethod} onChange={(event) => { setPaymentMethod(event.target.value); setPendingOverride(false); setPaymentEvidenceFiles([]); }}><option value="EFECTIVO">Efectivo</option><option value="TRANSFERENCIA">Transferencia</option><option value="QR">QR</option>{saleType === "HUESPED" && <option value="PENDIENTE">Cargo pendiente</option>}<option value="CORTESIA">{isAdmin ? "Cortesía autorizada" : "Solicitar cortesía"}</option><option value="OTRO">Otro</option></select></label>{["TRANSFERENCIA", "QR", "OTRO"].includes(paymentMethod) && <><label>Referencia del pago digital<input value={paymentReference} onChange={(event) => setPaymentReference(event.target.value)} placeholder="N.º de operación o respaldo" /></label><label>Captura o PDF opcional<input type="file" accept="image/*,application/pdf" multiple onChange={(event) => setPaymentEvidenceFiles(Array.from(event.target.files || []).slice(0, 3))} /><small>{paymentEvidenceFiles.length ? `${paymentEvidenceFiles.length} respaldo(s) seleccionado(s)` : "Máximo 3 archivos de 15 MB"}</small></label></>}<label>Observaciones<textarea value={notes} onChange={(event) => setNotes(event.target.value)} placeholder={paymentMethod === "CORTESIA" ? "Motivo de la cortesía…" : "Referencia o detalle opcional…"} required={paymentMethod === "CORTESIA"} /></label></div>
    ...
    {selectedSale && <SaleOperations sale={selectedSale} items={data.saleItems.filter((item) => item.sale_id === selectedSale.id)} payments={data.payments.filter((payment) => payment.sale_id === selectedSale.id)} evidences={data.paymentEvidences} cashOpen={Boolean(data.currentCashSession)} onClose={() => setSelectedSaleId(null)} onReload={load} onNotice={setNotice} />}

    {isAdmin && data.recentSales.some((sale) => sale.status === "CORTESIA_PENDIENTE") && <section className="panel courtesy-approvals"><div className="panel-head"><div><h2>Cortesías por aprobar</h2><p>El stock aún no fue descontado.</p></div><span>{data.recentSales.filter((sale) => sale.status === "CORTESIA_PENDIENTE").length} pendientes</span></div>{data.recentSales.filter((sale) => sale.status === "CORTESIA_PENDIENTE").map((sale) => <article key={sale.id}><div><b>{sale.sale_number} · {money(sale.total_cents)}</b><small>{sale.sale_type === "HUESPED" ? `Habitación ${sale.room_number}` : sale.customer_name || "Venta directa"} · solicitada por {sale.created_by_name}</small></div><div><button className="danger-light" disabled={busy} onClick={() => reviewCourtesy(sale, "RECHAZADA")}>Rechazar</button><button className="primary" disabled={busy} onClick={() => reviewCourtesy(sale, "APROBADA")}>Aprobar cortesía</button></div></article>)}</section>}

    <section className="panel pos-history"><div className="panel-head"><div><h2>Ventas recientes</h2><p>Comprobantes, saldos, cobros parciales y devoluciones visibles.</p></div><span>{data.recentSales.length} registros</span></div><div className="pos-sales-table"><table><thead><tr><th>Número</th><th>Destino</th><th>Pago</th><th>Estado</th><th>Total / saldo</th><th>Responsable</th><th>Acciones</th></tr></thead><tbody>{data.recentSales.map((sale) => { const lockedCourtesy = ["CORTESIA_PENDIENTE", "CORTESIA_RECHAZADA"].includes(sale.status); return <tr key={sale.id} className={["ANULADA", "CORTESIA_RECHAZADA"].includes(sale.status) ? "cancelled" : ""}><td><b>{sale.sale_number}</b><small>{new Intl.DateTimeFormat("es-BO", { dateStyle: "short", timeStyle: "short" }).format(new Date(sale.created_at))}</small></td><td>{sale.sale_type === "HUESPED" ? `Hab. ${sale.room_number}` : sale.customer_name || "Venta directa"}</td><td>{paymentLabels[sale.payment_method] || sale.payment_method}<small>Pagado {money(sale.paid_cents)}</small></td><td><span className={`sale-status ${sale.status.toLowerCase()}`}>{saleStatusLabels[sale.status] || sale.status}</span>{sale.cancellation_reason && <small>{sale.cancellation_reason}</small>}{sale.courtesy_review_note && <small>{sale.courtesy_review_note}</small>}</td><td><b>{money(sale.total_cents)}</b><small>Saldo {money(sale.balance_cents)}</small></td><td>{sale.created_by_name}</td><td><div className="sale-actions">{!lockedCourtesy && <a href={`/api/store?receipt=${encodeURIComponent(sale.sale_number)}`} target="_blank" rel="noreferrer">{sale.print_count ? "Reimprimir" : "Imprimir"}</a>}{!["ANULADA", "DEVUELTA"].includes(sale.status) && !lockedCourtesy && <button onClick={() => setSelectedSaleId(sale.id)}>Cobrar / devolver</button>}{sale.status !== "ANULADA" && !lockedCourtesy && <button disabled={busy} onClick={() => cancelSale(sale)}>Anular</button>}</div></td></tr>; })}</tbody></table></div></section>
...
function SaleOperations({ sale, items, payments, evidences, cashOpen, onClose, onReload, onNotice }: { sale: Sale; items: SaleItem[]; payments: Payment[]; evidences: PaymentEvidence[]; cashOpen: boolean; onClose: () => void; onReload: () => Promise<void>; onNotice: (message: string) => void }) {
  ...
  const pay = async (event: FormEvent) => { event.preventDefault(); try { const result = await post({ action: "sale_payment", saleId: sale.id, amountCents: Math.round(Number(amount) * 100), paymentMethod, reference }); let evidenceWarning = ""; if (result.paymentId && evidenceFiles.length) { try { await uploadPaymentEvidences(result.paymentId, evidenceFiles); } catch { evidenceWarning = " El cobro sí quedó guardado, pero el respaldo digital no pudo cargarse."; } } await onReload(); onNotice(`Cobro registrado. Saldo restante: ${money(result.balanceCents)}.${evidenceWarning}`); onClose(); } catch (error) { onNotice(error instanceof Error ? error.message : "No se pudo registrar el cobro."); } };
...
function CommercialReports({ reports }: { reports: ReportData }) {
  ...
  return <section className="commercial-reports"><div className="panel-head"><div><h2>Resumen comercial</h2><p>Visible únicamente para Administración y Propietario.</p></div><div className="report-export-links"><a href="/api/commercial-reports?type=sales">Exportar ventas</a><a href="/api/commercial-reports?type=inventory">Exportar almacén</a></div></div><div className="report-periods">{reports.periods.map((period) => <article className="panel" key={period.period}><span>{period.period}</span><strong>{money(period.net_sales_cents)}</strong><small>{period.sale_count} ventas netas</small></article>)}</div><div className="report-grid"><article className="panel"><h3>Productos y servicios más vendidos</h3><div className="report-bars">{reports.products.map((product) => <div key={product.product_id}><header><b>{product.product_name}</b><span>{product.units} un. · {money(product.revenue_cents)}</span></header><i><em style={{ width: `${Math.max(4, Number(product.units) / maxUnits * 100)}%` }} /></i><small>Ganancia aproximada: {money(Number(product.revenue_cents) - Number(product.cost_cents))}</small></div>)}{!reports.products.length && <p>Sin ventas todavía.</p>}</div></article><article className="panel"><h3>Ventas por trabajador</h3><div className="report-list">{reports.workers.map((worker) => <div key={worker.created_by_name}><span><b>{worker.created_by_name}</b><small>{worker.sale_count} operaciones</small></span><strong>{money(worker.sales_cents)}</strong></div>)}</div><h3>Formas de pago</h3><div className="report-list">{reports.paymentMethods.map((method) => <div key={method.payment_method}><span><b>{paymentLabels[method.payment_method] || method.payment_method}</b><small>{method.payment_count} cobros</small></span><strong>{money(method.amount_cents)}</strong></div>)}</div></article><article className="panel"><h3>Ventas por huésped y habitación</h3><div className="report-list">{reports.guestsRooms.map((item) => <div key={`${item.consumer_name}-${item.room_number}`}><span><b>{item.consumer_name}</b><small>Hab. {item.room_number} · {item.sale_count} operaciones</small></span><strong>{money(item.total_cents)}</strong></div>)}</div></article><article className="panel"><h3>Estados de venta</h3><div className="report-list">{reports.statuses.map((item) => <div key={item.status}><span><b>{saleStatusLabels[item.status] || item.status}</b><small>{item.sale_count} registros</small></span><strong>{money(item.total_cents)}</strong></div>)}</div></article></div></section>;
}
