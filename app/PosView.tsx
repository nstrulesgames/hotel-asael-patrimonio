...
type PosData = { user: { id: number; name: string; role: string }; products: Product[]; activeStays: ActiveStay[]; occupants: Occupant[]; recentSales: Sale[]; pendingByStay: Array<{ stay_id: number; sale_count: number; pending_cents: number }>; pendingLimitCents: number; currentCashSession?: CashSession; cashSessions: CashSession[]; saleItems: SaleItem[]; payments: Payment[]; paymentEvidences: PaymentEvidence[]; returns: Array<Record<string, unknown>>; reports: ReportData };
...
export default function PosView() {
  ...
  const submit = async (event: FormEvent) => {
    ...
    try {
      ...
      setCart({}); setNotes(""); setCustomerName(""); setPaymentReference(""); setPaymentEvidenceFiles([]); setPendingOverride(false);
      setLastSale({ saleNumber: result.saleNumber, receiptUrl: result.receiptUrl });
      setNotice(`${result.saleNumber} registrada por ${money(result.totalCents)}.${evidenceWarning}`);
      await load();
